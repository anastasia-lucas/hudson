...
#' Suggested: ggrepel
#' @param top data frame, columns one and two must be 'Variable', 'pvalue', and 'Group';
#'            'Shape' and 'Color' optional
#' @param bottom data frame, columns one and two must be 'Variable', 'pvalue', and 'Group'; 
#'               'Shape and 'Color' optional
...
#' @param yaxis label for y-axis in the format c("top", "bottom"), 
#'              automatically set if log10=TRUE
#' @param opacity opacity of points where 0 < opacity <= 1, useful for dense plots
...
#' @param annotate_var vector of variable names to annotate
...
#' @param highlighter color to use for highlight
...
#' @param type plot type/extension, 'pdf' or 'png', etc.
#' @param hgtratio height ratio for top plot; bottom will be auto assigned to 1-hgtratio
...
emirror <- function(top, bottom, tline, bline, log10=TRUE, yaxis, opacity=1, 
  ...
  if(!missing(annotate_var)){
    if (!requireNamespace(c("ggrepel"), quietly = TRUE)==TRUE){
      print("Consider installing 'ggrepel' for improved text annotation")
      p1 <- p1 + geom_text(data=d_order[d_order$Variable %in% annotate_var & d_order$Location=="Top",], 
                           aes(pos_index,
                               pval,
                               label=Variable))
      p2 <- p2 + geom_text(data=d_order[d_order$Variable %in% annotate_var & d_order$Location=="Bottom",], 
                           aes(pos_index,
                               pval,
                               label=Variable))
    } else {
      p1 <- p1 + ggrepel::geom_text_repel(data=d_order[d_order$Variable %in% annotate_var & d_order$Location=="Top",], 
                                          aes(pos_index,
                                              pval,
                                              label=Variable))
      p2 <- p2 + ggrepel::geom_text_repel(data=d_order[d_order$Variable %in% annotate_var & d_order$Location=="Bottom",], 
                                          aes(pos_index,
                                              pval,
                                              label=Variable))
    }
  ...
  if(grpblocks==TRUE){
    ...
    p1 <- p1 + theme(axis.text.x = element_text(vjust=1),
                     axis.ticks.x = element_blank()) + 
      scale_y_continuous(limits=c(yaxismin1, yaxismax1),
