#' iphemirror
#'
#' Create interactive mirrored Manhattan plots for PheWAS
#' Dependencies: ggplot2, ggiraph
#' Suggested: ggrepel
#' @param top data frame, must contain PHE, SNP, CHR, POS, pvalue columns, optional Shape, Hover, Link
#'                        where 'Hover' contains tooltip information overriding the default (SNP, chr:position, Phenotype)
#'                        and 'Link' contains a search query, ex. "https://www.ncbi.nlm.nih.gov/snp/rs944173"
#' @param bottom data frame, must contain PHE, SNP, CHR, POS, pvalue, columns, optional Shape, Hover, Link
#'                           where 'Hover' contains tooltip information overriding the default (SNP, chr:position, Phenotype)
#'                           and 'Link' contains a search query, ex. "https://www.ncbi.nlm.nih.gov/snp/rs944173"
...
iphemirror <- function(top, bottom, phegroup, tline, bline, chroms = c(1:22,"X","Y"),
                       ...
                       background="variegated", chrblocks=TRUE, file="iphemirror", 
  ...
  d_index <- unique(d_phe[,colnames(d_phe) %in% c("CHR", "POS")])
  d_order <- d_index[order(d_index$CHR, d_index$POS), ]
  d_order$pos_index <- seq.int(nrow(d_order))
  #d_order_sub <- d_order[colnames(d_order) %in% c("SNP", "CHR", "POS", "pvalue", "pos_index")]
  d_order_sub <- d_order[, !(colnames(d_order) %in% c("Hover", "Link"))]
  ...
  names(lims) <- c("Color", "snpx", "posmin", "posy", "posmax")
  lims$av <- (lims$posmin + lims$posmax)/2
  lims <- lims[order(lims$Color),]
  lims$shademap <- rep(c("shade_ffffff", "shade_ebebeb"), length.out=nrow(lims), each=1)
  
  #d_order <- d_order[which(d_order$Data!="GWAS"),]
  d_order <- merge(d_phe, d_order_sub, by=c("CHR", "POS"))
  ...
  newcols <- Turbo(out.colors=ngroupcolors)
